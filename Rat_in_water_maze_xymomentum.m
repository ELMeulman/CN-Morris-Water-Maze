...
rng(2); % set seed for repeatable random numbers
...
% ACTOR INFO
theta=[0:1:7]/8*2*pi-pi; % angle directions for each of the 8 actions
dx=v_rat*dt*sin(theta); % dx directions for each of the 8 actions
dy=v_rat*dt*cos(theta); % dy directions for each of the 8 actions
zij=zeros(ni,nj);
...
for itrial=1:ntrial
    goal=0;
    t_rat=2*pi*(randi(4)-1)/4;%rand*2*pi; % position of rat starting anywhere along the edge of the pool
    ...
    dx_prev=v_rat*dt*sin(-t_rat); % initialize previous heading
    dy_prev=v_rat*dt*cos(-t_rat); % initialize previous heading
    ...
    for it=1:nt
        ...
        % Momentum
        dxj=(dx(j)+momentum*dx_prev)/(momentum+1);
        dyj=(dy(j)+momentum*dy_prev)/(momentum+1);
        xj=x_rat(it,itrial)+dxj; 
        yj=y_rat(it,itrial)+dyj;
        rj=sqrt(xj.^2+yj.^2);
        ...
        xj=x_rat(it,itrial)+dxj; 
        yj=y_rat(it,itrial)+dyj;
        x_rat(it+1,itrial)=xj;
        y_rat(it+1,itrial)=yj;
        dx_prev=dxj;
        dy_prev=dyj;
        % CRITIC
        fi_next=f(x_rat(it+1,itrial),y_rat(it+1,itrial),1:ni);
        C=wi'*fi; % weights have changed so recalculate C for current location. 
        C_next=wi'*fi_next; % weights have changed so recalculate C for next location. 
        ...
        wi=wi+epsilon*delta*fi; % formula (8), update wi for all neurons
        zij(:,j)=zij(:,j)+epsilon*delta*fi; % formula (10), update z for chosen action j = nearest angle ii(j,1)
    ...
    for ix=1:n_grid
        for iy=1:n_grid
            ...
            C_grid(itrial,ix,iy)=wi'*fi;
        end
    end
    C_grid(itrial,:,:)=C_grid(itrial,:,:)/max(max(C_grid(itrial,:,:)));
    wi_trial(:,itrial)=wi;%/max(wi);
...
%scatter(axpa2,xi,yi,abs(wi_trial(:,2))*1000);
%scatter(axpa7,xi,yi,abs(wi_trial(:,7))*1000);
%scatter(axpa22,xi,yi,abs(wi_trial(:,22))*1000);
